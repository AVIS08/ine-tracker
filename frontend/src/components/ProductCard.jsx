  ...
  return (
    <div className="product-card">
      <Link
        ...
        style={{ textDecoration: 'none', color: 'inherit', display: 'flex', flexDirection: 'column', gap: 'var(--space-md)', flex: 1 }}
      >
        ...
        <div className="product-card-stock">
          {stockStatus === 'in-stock' ? (
            <span className="stock-pill in">● In Stock{stock > 0 ? ` (${stock})` : ''}</span>
          ) : stockStatus === 'out-of-stock' ? (
            <span className="stock-pill out">● Out of Stock</span>
          ) : (
            <span style={{ color: 'var(--text-dim)', fontSize: '0.8rem' }}>● Stock Unknown</span>
          )}
        </div>
      ...
      <div className="product-card-footer">
        <div className="product-card-meta">
          ...
          {latestLog && (
            <span>
              {' · '}
              Scraped {formatTime(latestLog.scraped_at)}
              {' · '}
              <span className={`status-badge status-${latestLog.status}`}>
                {latestLog.status}
              </span>
            </span>
          )}
